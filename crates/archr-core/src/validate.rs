...
use crate::model::{ElementKind, ElementLayer, Model, RelationKind};
use serde::Serialize;
...
fn is_allowed(
    source_layer: ElementLayer,
    rel_kind: RelationKind,
    target_layer: ElementLayer,
) -> bool {
    ALLOWED
        .iter()
...
pub fn validate_relationship(
    ...
    if !is_allowed(source_layer, rel_kind, target_layer) {
        Err(ValidationError {
            ...
            suggestion: "Change relation type or ensure source/target are compatible".to_string(),
        })
    } else {
        Ok(())
    }
...
mod tests {
    ...
    fn test_valid_serving() {
        ...
        let component =
            model.add_element("ApplicationComponent", ElementKind::ApplicationComponent);
    ...
    fn test_invalid_realization_crosslayer() {
        ...
        assert_eq!(result.errors[0].element_source, "BusinessActor");
    }
    ...
    fn test_serving_valid_tech_app() {
        ...
        assert!(
            result.success,
            "Technology→Application Serving should be valid"
        );
    }
    ...
    fn test_serving_valid_tech_business() {
        ...
        assert!(
            result.success,
            "Technology→Business Serving should be valid"
        );
    }
    ...
    fn test_serving_valid_app_business() {
        ...
        assert!(
            result.success,
            "Application→Business Serving should be valid"
        );
    }
    ...
    fn test_serving_valid_app_strategy() {
        ...
        assert!(
            result.success,
            "Application→Strategy Serving should be valid"
        );
    }
    ...
    fn test_serving_invalid_business_app() {
        ...
        assert!(
            !result.success,
            "Business→Application Serving should be invalid"
        );
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].code, "INVALID_RELATIONSHIP");
    }
    ...
    fn test_serving_invalid_app_tech() {
        ...
        assert!(
            !result.success,
            "Application→Technology Serving should be invalid"
        );
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].code, "INVALID_RELATIONSHIP");
    }
    ...
    fn test_serving_invalid_same_layer_tech() {
        ...
        assert!(
            !result.success,
            "Technology→Technology Serving should be invalid"
        );
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].code, "INVALID_RELATIONSHIP");
    }
    ...
    fn test_serving_invalid_same_layer_business() {
        ...
        assert!(
            !result.success,
            "Business→Business Serving should be invalid"
        );
        assert_eq!(result.errors.len(), 1);
        assert_eq!(result.errors[0].code, "INVALID_RELATIONSHIP");
    }
